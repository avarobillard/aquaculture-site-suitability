...
```{r}
# Transform EEZ data to correct CRS
eez <- st_transform(eez, crs(suitability))
...
# Subset EEZ based on mask
masked_suitability <- mask(rmask, eez)

# Set all cells with suitability score of 0 to NA
masked_suitability[masked_suitability == 0] <- NA

# Find suitable area 
suitability_km <- cellSize(masked_suitability, unit = "km", mask = TRUE)

# Find total suitable area within each EEZ 
cell_area <- zonal(suitability_km, eez_rast, fun = "sum", na.rm = TRUE) %>%
  as.data.frame() %>% 
...
# Join suitable cell area data frame to EEZ for plotting
eez_suitability <- eez %>% 
...
Based upon these maps, we can determine which Exclusive Economic Zones might be best suited for developing marine aquaculture for a species of interest. For example, the map we created for Rainbow Trout indicated that the Washington EEZ contains the most suitable area at 700-800 km^2^, followed by the Southern California EEZ with 600-699 km^2^ suitable area. One limitation of this analysis is that it only included two environmental variables, mean sea surface temperature (SST) and depth. Additional environmental factors, such as salinity and food availability, could also influence the amount of suitable habitat. Additionally, the way SST was calculated in this analysis might cause different results, as the SST was averaged over 5 years (2008-2012), but climatic events such as El Niño and La Niña could cause significant temperature changes during that time. For real-world applications, it might be beneficial to incorporate limitations on which species are best for aquaculture, such as their conservation status and whether the species is native to the West Coast, to ensure that aquaculture is developed responsibly. 
